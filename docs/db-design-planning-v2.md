...
>
> **Create update (2026-07-22):** `flows.surface` distinguishes the direct
> Create session list from the canvas library. It is presentation metadata only;
> both values use the same graph, snapshots, jobs, and canonical Assets.
...
create index "flowsOrgSurfaceUpdatedIdx"
  on "flows" ("organizationId", "surface", "updatedAt" desc, "id" desc);
create unique index "flowsAssetFolderIdx"
  on "flows" ("assetFolderId") where "assetFolderId" is not null;
```

The surface discriminator scopes server-side browsing only. A Create session
stores an ordinary small Flow graph and every submission remains an ordinary
immutable Flow run. No Create request, message, attachment, result, planner, or
job table exists. `Open in Flow` copies the current normalized graph into a new
canvas Flow while historical runs stay attached to the source identity.
...
existing generation-ingestion path and is placed in the generating Flow's
stable `Flow/<Flow name>` Asset folder. Uploads are not included in this rule.
