...
>
> **Create update (2026-07-23):** Create is a browser-local direct-generation
> playground. It has no Flow/session/graph identity and never writes a draft to
> `flows`, `flowNodes`, or `flowEdges`. Its durable runs use source `create`,
> `flowId = null`, the shared immutable execution plan, ordinary generation
> jobs, and canonical Assets.
...
discriminator, Create Flow, Create-session query, or clone-to-canvas operation.
The Create draft stays in browser-local same-tab recovery storage. Create
execution reuses the run and job tables below without a Flow foreign key; there
is no Create draft, graph, message, attachment, result, planner, or job table.
...
create -> no Flow identity + frozen direct request + ExecutionPlan
```

Create uses mode `direct` and `flowId = null`. Both sources persist through the
same run, step, item, job, source, input, accounting, and output rows.
...
create table "flowRuns" (
  ...
  check (
    ("source" = 'create' and "flowId" is null and "mode" = 'direct'
      and "targetNodeId" is null)
    or ("source" = 'flow' and "mode" <> 'direct')
  ),
...
create index "flowRunsCreateHistoryIdx"
  on "flowRuns" ("organizationId", "createdBy", "createdAt" desc, "id" desc)
  where "source" = 'create';
