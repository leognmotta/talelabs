...
>
> **Create update (2026-07-23):** Create is a direct-generation playground with
> lightweight durable session identity and browser-local drafts. It never writes
> a Flow or graph to `flows`, `flowNodes`, or `flowEdges`. Its durable runs use
...
`flowRuns` row wrapping ordinary `generationJobs`. The historical table names
remain, while current runtime contracts call the executable units generic
steps. Future Tools reuse this spine. Credit-system design has its own planning
...
- Archive and purge both exist on `assets` — see the two-tier deletion note
  there. `createSessions.deletedAt` is a presentation-only soft deletion that
  preserves immutable runs and generated Assets. Everything else hard-deletes.
...
erDiagram
    ...
    createSessions ||--o{ flowRuns : "groups direct runs"
...
create unique index "flowsAssetFolderIdx"
  on "flows" ("assetFolderId") where "assetFolderId" is not null;
```

Every row is an ordinary spatial Flow. There is no presentation-surface
discriminator, Create Flow, or clone-to-canvas operation. The Create draft stays
in browser-local same-tab recovery storage. A small `createSessions` row gives
direct requests a stable route and history boundary without becoming a Flow or
graph document. Create execution reuses the run and job tables below without a
Flow foreign key; there is no server-side Create draft, graph, message,
attachment, result, planner, or job table.

`assetFolderId` is the stable destination for media generated by that Flow.
The worker provisions it lazily as a collision-safe child of the internally
identified `Flow` root and assigns generated image/video/audio Assets in the
same transaction that inserts the Asset. Ordinary uploads never derive their
folder from a Flow. Folder moves remain safe because the relationship uses IDs;
deleting the destination clears the association and a later generated output
provisions a replacement.

### 2a. Create sessions — direct-history identity

Create sessions group related direct requests. They own no creative document,
draft, messages, graph, viewport, provider configuration, or generated files.
The first Generate from `/create` creates the session and first run atomically.
...
Listing and mutation are tenant- and creator-scoped. Deleting a session sets
`deletedAt`; it removes the session from Create navigation without deleting its
runs, generation provenance, or canonical Assets.
...
create table "generationJobs" (
  ...
  "nodeId" text not null,           -- historical column name for execution-step identity;
                                    -- Flow steps may retain node ids, direct Create does not
...
create table "generationJobSources" (
  ...
  "nodeId" text not null,             -- historical source identity column; Flow node or direct Asset
...
The run source is independent from that execution model:

```txt
flow   -> persisted Flow identity + frozen graph revision + ExecutionPlan
create -> lightweight session identity + frozen direct request + ExecutionPlan
```

Create uses mode `direct`, `flowId = null`, and a required `createSessionId`.
Both sources persist through the same run, step, item, job, source, input,
accounting, and output rows.
...
create table "flowRuns" (
  ...
  "createdBy" text references "user"("id") on delete set null,
  "flowId" text,
  "createSessionId" text,
  "source" text not null default 'flow'
           check ("source" in ('flow', 'create')),
  "mode" text not null check ("mode" in
    ('node', 'downstream', 'upstream', 'selection', 'all', 'tool', 'direct')),
  ...
  "graphSnapshot" jsonb not null default '{}',
        -- historical column name; current immutable envelope is:
        -- discriminated source + generic ExecutionPlan + exact private
        -- execution contracts. A Flow source freezes graph/revision evidence;
        -- a Create source freezes a bounded direct request and contains no graph.
  ...
  check (
    ("source" = 'create' and "flowId" is null
      and "createSessionId" is not null and "mode" = 'direct'
      and "targetNodeId" is null)
    or ("source" = 'flow' and "createSessionId" is null
      and "mode" <> 'direct')
  ),
  check ("cancellationReconciledAt" is null or "status" = 'canceled'),
  foreign key ("flowId", "organizationId")
    references "flows" ("id", "organizationId") on delete set null ("flowId"),
  foreign key ("createSessionId", "organizationId")
    references "createSessions" ("id", "organizationId") on delete restrict,
...
Per-step and per-item execution state are **rows, not mutated jsonb**.
`"graphSnapshot"` stays immutable. The historical physical table
`flowRunNodes` stores generic step summaries; `flowRunNodeItems` holds expanded
work-item dimensions. This keeps progress, partial failure, retry, and UI
queries relational without mutating the snapshot and avoids a second Create
execution engine.
...
plan order, graph edges in deterministic connection order for Flow sources, and
selected inputs in provider order) retain their order. The same serializer
powers request hashing, audit verification, and any later organization-scoped
snapshot deduplication; never hash ordinary `JSON.stringify` output assembled
from unordered maps.
...
create table "flowRunNodes" (
  ...
  "nodeId" text not null,               -- historical name for immutable execution-step id
  "status" text not null default 'pending'
...
existing generation-ingestion path. Flow outputs use the generating Flow's
stable `Flow/<Flow name>` Asset folder; direct Create outputs have no Flow
folder identity. Uploads are not included in this rule.
...
- **Create direct runs:** source `create` runs use mode `direct` and
  ...
  without creating a lightweight Flow, graph document, or parallel execution
  architecture.
