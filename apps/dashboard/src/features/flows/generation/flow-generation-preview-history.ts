/** Preview-history projection that preserves mounted media across run transitions. */

import type { FlowLatestResult } from '@talelabs/sdk'
import type { TFunction } from 'i18next'
import type { FlowGenerationPreview } from '../editor/flow-canvas-types'

import { previewFromOutputJobs } from '../runs/observation/flow-run-preview-projection'

/** Restores succeeded previews from the Flow's persisted latest-result projection. */
...
/** Retains mounted media objects while a refreshed run reports the same canonical output. */
