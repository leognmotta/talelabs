/** Local ephemeral selection owner for one mounted Asset library surface. */
...
import {
  getAssetDragSelection,
  getSelectedAssets,
} from './asset-library-selected-assets'
import {
...
/** Owns current Asset/folder selection and exposes selection-aware Asset adapters. */
export function useAssetLibrarySelection({
  ...
  return {
    clear,
    getAssetDragData: (asset: Asset) => getAssetDragSelection(
      assets,
      selectedAssetIdsSet,
      asset,
    ),
    getSelectedAssets: (asset: Asset) => getSelectedAssets(
      assets,
      selectedAssetIdsSet,
      asset,
    ),
