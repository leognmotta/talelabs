...
const GEMINI_TTS_VOICE_SETTING = {
  default: 'gemini-alloy',
  id: 'voice',
  kind: 'enum',
  labelKey: 'flows.settings.voice',
  options: [
    {
      labelKey: 'flows.settings.voices.geminiAlloy',
      value: 'gemini-alloy',
    },
  ],
} as const

/** Current OpenRouter speech model. Older speech contracts remain historical. */
export const GEMINI_31_FLASH_TTS_MODEL = {
  capabilitySchemaVersion: 3,
  constraints: [],
  defaultOperationId: 'textToSpeech',
  displayName: 'Gemini 3.1 Flash TTS Preview',
  enabled: true,
  id: 'talelabs/gemini-3.1-flash-tts-preview',
  inputSlots: [SCRIPT_INPUT],
  labelKey: 'flows.models.gemini31FlashTtsPreview',
  mediaType: 'audio',
  operations: [
    {
      descriptionKey: 'flows.operations.textToSpeechDescription',
  ...
  recommended: true,
  settings: [
    GEMINI_TTS_VOICE_SETTING,
    {
      ...AUDIO_OUTPUT_FORMAT_SETTING,
      options: AUDIO_OUTPUT_FORMAT_SETTING.options.filter(
