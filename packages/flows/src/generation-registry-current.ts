...
const CURRENT_NANO_BANANA_2_LITE_MODEL = fixedSingleImageOutput(
  NANO_BANANA_2_LITE_MODEL,
)
const CURRENT_NANO_BANANA_2_MODEL = fixedSingleImageOutput(
  NANO_BANANA_2_MODEL,
)
const CURRENT_NANO_BANANA_PRO_MODEL = fixedSingleImageOutput(
  NANO_BANANA_PRO_MODEL,
)
const CURRENT_GPT_IMAGE_2_MODEL = fixedSingleImageOutput(
  ADAPTIVE_GPT_IMAGE_2_MODEL,
)
const CURRENT_SEEDREAM_45_MODEL = withSeedreamAutoAspectRatio(
  fixedSingleImageOutput(SEEDREAM_45_MODEL),
)
const CURRENT_FLUX_2_PRO_MODEL = fixedSingleImageOutput(FLUX_2_PRO_MODEL)
const CURRENT_RECRAFT_41_MODEL = fixedSingleImageOutput(RECRAFT_41_MODEL)
...
} as const satisfies Readonly<Record<string, HardenedGenerationModelDefinition>>

function withNodeType(
  model: HardenedGenerationModelDefinition,
...
].map(model => withNodeType(model, 'imageGeneration'))

const CURRENT_LLM_MODELS = [
  GEMINI_31_FLASH_LITE_MODEL,
  CLAUDE_SONNET_46_MODEL,
  GPT_54_MODEL,
  GEMINI_31_PRO_MODEL,
  DEEPSEEK_V32_MODEL,
  MISTRAL_LARGE_3_MODEL,
].map(model => withNodeType(model, 'llm'))

const CURRENT_VIDEO_MODELS = [
  ADAPTIVE_VEO_31_MODEL,
  VEO_31_LITE_MODEL,
  GROK_IMAGINE_VIDEO_MODEL,
  SEEDANCE_20_MODEL,
  ADAPTIVE_LTX_23_PRO_MODEL,
].map(model => withNodeType(model, 'videoGeneration'))

/** Current intent-tagged creative catalog. */
export const CURRENT_GENERATION_MODEL_REGISTRY = Object.fromEntries([
  ...
  ELEVEN_VOICE_ISOLATOR_MODEL,
  STABLE_AUDIO_25_MODEL,
].map(model => [model.id, model])) as Readonly<
