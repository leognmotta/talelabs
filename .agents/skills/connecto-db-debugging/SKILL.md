---
name: connecto-db-debugging
description: Use when debugging Connecto database state, validating data, checking API database behavior, or running read-only PostgreSQL queries through the root POSTGRES_URL in this monorepo.
---

# Connecto DB Debugging
...
2. Prefer the app's exported `@connecto/db` client for application changes.
...
```bash
node .agents/skills/connecto-db-debugging/scripts/query-db.mjs "select now() as now"
```
...
```bash
node .agents/skills/connecto-db-debugging/scripts/query-db.mjs <<'SQL'
